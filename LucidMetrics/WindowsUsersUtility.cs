...
{
    /// <summary>
    /// Simple utility for retrieving a set of local logged on users.
    ///
    /// </summary>
    public class WindowsUsersUtility
    {
        ...
        /// <summary>
        /// Retrieves a list of logged on users
        /// </summary>
        /// <param name="ServerName">The name of the server to check.</param>
        /// <returns></returns>
        public static IEnumerable<(string Domain, string User)> ListUsers(String ServerName)
        {
            ...
            try
            {
                ...
                if (retVal != 0)
                {
                    for (int i = 0; i < sessionCount; i++)
                    {
                        ...
                        yield return (Marshal.PtrToStringAnsi(domainPtr), Marshal.PtrToStringAnsi(userPtr));
                    }
                    Console.ReadLine();
